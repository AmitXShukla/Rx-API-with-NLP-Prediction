...
export default class IntroWhisper {
  ...
  createComponents() {
    ...
    const collapsibleContent: whisper.Markdown = {
      ...
      Step 1: login to Healthcare OLTP / ERP Application, ClinicalTrails.gov_API_AI Loop is expecting you to use a your ERP app.

      Step 2: enter some data in app | search text in Olive Help | select & copy text to your clipboard
        for example, heart attack | stroke (use these strings for **demo)

      Step 3: AI API is smart to parse your text, perform an elastic search in database and render active matching results.

      Step 4: Review two sections in search results, Section 1: About/Details about searched searched text | Section 2: AI Alerts based on pre-trained ML Models.
      `,
    };
